...
def train():
    input_images = tf.placeholder(tf.float32, shape=[None, 210, 160, 4])
    ...
    with tf.Session() as sess, g.as_default():
        game_env = gym.make('BreakoutNoFrameskip-v3')
        observe = game_env.reset()
        sess.run(tf.global_variables_initializer())
        for round in xrange(BURN_OUT_ROUND):
            tf
        ...
        for episode in xrange(NUM_OF_EPISODE):
            ...
            random_action = tf.random_uniform([1], minval=0, maxval=NUM_OF_ACTION-1)
